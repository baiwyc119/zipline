...
def doLaunch(total, stock, title, context=None):
    ...
    if context:  # 买单
        ...
        if assign > 0:
            direct = order_value(stock, assign)
        else:
            result['enough'] = True
    else:  # 卖单
    ...
    # 记录日志------------------------------------------------------------------
    if direct:
        print '%s：stock=%s,下单数量=%d,成交数量=%d,成交价格=%.2f,成交金额=%.2f,目标金额=%.2f' % (
        title, stock, direct.amount, direct.filled, direct.price, direct.filled * direct.price, assign)
    else:
